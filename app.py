...
def render_detail(result, strategy, interval):
    ...
    if result["details"]:
        ...
        for i,(k,v) in enumerate(result["details"].items()):
            dc[i%3].markdown(
                f'<div class="mc"><div class="ml">{k}</div>'
...
# ── SESSION STATE ─────────────────────────────────────────────────
defaults = {"strategy":None,"selected_ticker":None,"scan_done":False,
            "results":[],"page":"scanner","bt_done":False,"bt_results":{},
            "bm_df":None,"search_result":None}
...
with st.sidebar:
    ...
    st.markdown("---")
    if st.button("📊 Performans", use_container_width=True, type="primary", key="perf_btn"):
...
with col1:
    if st.button("🟡 Emre'nin Stratejisi", use_container_width=True, type="primary"):
        st.session_state.update(strategy="emre", selected_ticker=None,
                                scan_done=False, page="scanner", results=[])
        st.rerun()
with col2:
    if st.button("🟣 Momentum Kırılımcısı", use_container_width=True):
        st.session_state.update(strategy="momentum", selected_ticker=None,
                                scan_done=False, page="scanner", results=[])
        st.rerun()
    ...
    for col, sk in [(col1,"emre"),(col2,"momentum")]:
        with col:
            ...
            for k,(passed,val) in crit.items():
                icon="✅" if passed else "❌"
                cc="pass" if passed else "fail"
                st.markdown(f'<div class="mc"><div class="ml">{k}</div>'
                            f'<div class="mv {cc}">{icon} {val}</div></div>',
                            unsafe_allow_html=True)
    st.markdown("---")
...
if st.session_state.page == "sektor":
    ...
    def cat_card(title, items, bg, border):
        ...
        for sect,val in items:
            ...
            rows+=(f'<div style="display:flex;justify-content:space-between;padding:.3rem 0;'
                   f'border-bottom:1px solid #1e2535;font-size:.83rem;">'
                   f'<span style="color:#e2e8f0">{sect}</span>'
                   f'<span style="color:{vc};font-family:JetBrains Mono;font-weight:600">{val}</span></div>')
        return (f'<div style="background:{bg};border:1px solid {border};border-radius:10px;'
                f'padding:1rem 1.2rem;margin-bottom:.8rem">'
                f'<div style="font-size:.7rem;font-weight:700;color:{border};text-transform:uppercase;'
...
if st.session_state.page == "macro":
    ...
    if one_cikanlar:
        ...
        for col,tkr in zip(oc_cols,one_cikanlar):
            col.markdown(f'<div class="mc" style="text-align:center">'
                        f'<div style="font-weight:700;color:#f1f5f9">{tkr}</div>'
                        f'<div style="font-size:.65rem;color:#64748b">{get_sector(tkr)}</div>'
    ...
    for ticker,df in td.items():
        ...
        try:
            ...
            c=_c(df); bm=_c(bm_df)
            rs=rs_score(c,bm,20)
            rows.append({'Hisse':ticker.replace('.IS',''),'Sektör':get_sector(ticker),
                'Emre':f"{sc_e}/{mx_e}",'Momentum':f"{sc_m}/{mx_m}",
                'RS (20g)':f"{rs*100:.1f}%" if not np.isnan(rs) else "N/A",
                'Fiyat':f"₺{float(c.iloc[-1]):.2f}",
                '_es':sc_e,'_rs':rs if not np.isnan(rs) else -999})
        except Exception:
            pass
    ...
    if rows:
        ...
        if sel_t:
            ...
            if tkr_yf in td:
                ...
                render_detail({"ticker":tkr_yf,"score":sc,"max_score":mx,
                               "criteria":crit,"details":det,"df":td[tkr_yf]},"emre",interval)
    else:
...
if st.session_state.page == "perf":
    ...
    st.markdown("Her ayın ilk borsa günü top 5 hisse · 100.000 ₺ başlangıç")
    st.markdown("---")

    if st.button("▶️ Backtest Çalıştır (Haz 2024 →)", type="primary"):
        with st.spinner("2 yıllık veri çekiliyor (1-2 dakika)..."):
        ...
        for i,sk in enumerate(["emre","momentum"]):
            ...
            bt_results[sk]={"pv":pv,"bm":bm_n,"trades":trades,"active":active,
                            "monthly":monthly,
    ...
    if not bt_results:
        st.info("▶️ Backtest çalıştır butonuna bas.")
        st.stop()
    ...
    for col,sk in zip([c1,c2],["emre","momentum"]):
        with col:
            ...
            for pos in sorted(bt_results[sk].get("active",[]),key=lambda x:x['pnl_pct'],reverse=True):
                ...
                st.markdown(f'<div class="mc"><div style="display:flex;justify-content:space-between">'
                            f'<span style="font-weight:700;color:#f1f5f9">{pos["ticker"]}</span>'
                            f'<span class="stag">{sect}</span>'
                            f'<span style="color:{pc};font-family:JetBrains Mono;font-weight:600">{pnl:+.1f}%</span>'
                            f'</div><div style="font-size:.7rem;color:#64748b;margin-top:.2rem">'
    ...
    for col,sk in zip([s1,s2],["emre","momentum"]):
        with col:
            ...
            for k,v in bt_results[sk].get("stats",{}).items():
                is_pos="+" in str(v) and "Drawdown" not in k
                is_neg=str(v).startswith("-") or "Drawdown" in k
                vc="#22c55e" if is_pos else "#ef4444" if is_neg else "#f1f5f9"
                st.markdown(f'<div class="mc"><div class="ml">{k}</div>'
                            f'<div class="mv" style="color:{vc}">{v}</div></div>',
                            unsafe_allow_html=True)

    st.markdown("---")
    st.markdown("### 📅 Aylık Portföy Detayı")
    t1,t2=st.tabs([STRATEGY_FN["emre"][1],STRATEGY_FN["momentum"][1]])
        ...
        with tab:
            ...
            if mdf is not None and len(mdf)>0:
                def color_pnl(val):
                    ...
                    if isinstance(val,str) and val.startswith('-'): return 'color:#ef4444'
                    return ''
                cols_s=['Aylık P&L'] if 'Aylık P&L' in mdf.columns else []
                st.dataframe(mdf.style.map(color_pnl,subset=cols_s),
    ...
    for tab,sk in zip([lt1,lt2],["emre","momentum"]):
        with tab:
            ...
            if trades is not None and len(trades)>0:
                ...
                if len(sales): st.dataframe(sales.reset_index(drop=True),
...
if st.session_state.strategy is None:
    st.info("👆 Yukarıdan bir strateji seç.")
    st.stop()
...
if not st.session_state.scan_done:
    with st.spinner("Benchmark çekiliyor..."):
        bm_df = fetch_benchmark(period, interval)
        st.session_state.bm_df = bm_df
    with st.spinner(f"{len(BIST100_TICKERS)} hisse taranıyor..."):
...
with left_col:
    ...
    if not top5_r:
        st.markdown("*Filtreden geçen hisse yok.*")
    for r in top5_r:
...
with right_col:
    ...
    if not sel:
        ...
        if top5_r:
            ...
            for r in top5_r:
                st.markdown(f"⭐ **{r['ticker'].replace('.IS','')}** `{r['score']}/{r['max_score']}` · {r.get('sector','')}")
    else:
